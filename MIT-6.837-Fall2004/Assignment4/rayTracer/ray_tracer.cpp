    ...
    if (items -> intersect(ray, hit, tmin)) {
        ...
        if (shadows) {
            for (int i=0; i<sp->getNumLights(); i++) {
                ...
                if (!items -> intersect(shadowRay, shadowHit, EPSILON)) {
                    color += hit.getMaterial()->Shade(ray, hit, directionToLight, lightColor);
                }
                else if (transparent_shadows && shadowHit.getMaterial()->getTransparentColor().Length() > 0) {
                    ...
                    while (items -> intersect(shadowRay, shadowHit, newStart+EPSILON)) {
