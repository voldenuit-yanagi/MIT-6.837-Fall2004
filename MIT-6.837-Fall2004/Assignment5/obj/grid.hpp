...
struct MarchingInfo {
    ...
    void nextCell() {
        float min_next = min({t_next_x, t_next_y, t_next_z});
        if (min_next == t_next_x) {
            i += sign_x;
            tMin = t_next_x;
            t_next_x += d_tx;
            if (sign_x > 0) {
...
public:
    Grid(BoundingBox *bb, int nx_, int ny_, int nz_) {
        ...
        seqIndicativeMaterials[0] = new PhongMaterial(Vec3f(1, 1, 1), Vec3f(0, 0, 0), 0,
                                                   Vec3f(0, 0, 0), Vec3f(0, 0, 0), 0);
        for (int i=1; i<16; i++) {
            numIndicativeMaterials[i] = new PhongMaterial(Vec3f(rand()%106+250, rand()%106+250, rand()%106+250)*(1./256.), Vec3f(0, 0, 0), 0, Vec3f(0, 0, 0), Vec3f(0, 0, 0), 0);
            if (i<11) {
