...
void RayTracingStats::Initialize(int _width, int _height, BoundingBox *_bbox, 
  ...
  start_time = (time.tv_sec * 1000) + (time.tv_usec / 1000); // millisecond
...
void RayTracingStats::PrintStatistics()  {
  ...
  timeval time;
  gettimeofday(&time, NULL);
  int delta_time = (time.tv_sec * 1000) + (time.tv_usec / 1000) - start_time;
  if (delta_time == 0) delta_time = 1;
  int millis = delta_time % 1000;
  int secs  = (delta_time / 1000) % 60;
  int min   = (delta_time / (1000*60)) % 60;
  int hours = (delta_time / (1000*60));
  ...
  printf ("  total time                 %ld:%02d:%02d:%d\n",hours,min,secs, millis);
